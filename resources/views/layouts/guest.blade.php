...
<head>
    ...
    <style>
        :root{
            ...
            --acc:#18d893; --acc2:#00e3a5; --gold:#f0b90b; --red:#ef4444; --rad:16px;
        }
        *{box-sizing:border-box}
        ...
        .auth-bg{position:fixed;inset:0;z-index:0;background:
            radial-gradient(560px 360px at 12% 0%,rgba(24,216,147,.13),transparent 60%),
...
<body>
    ...
    <div class="auth-wrap">
        <a href="{{ url('/') }}" class="pv-logo">
            ...
            <span>Prime<span style="color:var(--acc)">Vest</span></span>
        </a>
        <div class="pv-card">
            @if (session('status'))
                <div style="background:rgba(24,216,147,.1);border:1px solid rgba(24,216,147,.3);color:var(--acc);padding:12px 14px;border-radius:10px;font-size:.88rem;margin-bottom:18px">{{ session('status') }}</div>
            @endif
            @if ($errors->any())
